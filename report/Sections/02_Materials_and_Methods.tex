...
Because the trained system is meant to be applied to recordings it has never
seen, each fold's weights are exported together with the per-channel mean and
standard deviation of \emph{that fold's training subjects} and its calibrated
threshold. At inference a new recording is trimmed, tokenised and normalised
with those stored statistics -- never with statistics recomputed from the new
cohort, which would leak the target distribution into its own predictions --
and the five fold models' window scores are averaged. We verified that the
windows rebuilt from a raw file at inference are numerically identical to the
training windows of the same subject, so the deployed path cannot silently
