...
\begin{figure*}[t]
  ...
  sequences -- one token per second, holding 11 statistics for each of the 18
  channels; the Mamba encoder produces a window score, and the scores of a
  subject's windows are averaged into one decision. Normalisation statistics
  and every hyper-parameter are estimated on training and validation data
  only, and each of the five subject-wise folds contributes a disjoint set of
  test subjects.}
  \label{fig:pipeline}
\end{figure*}
...
stride-time increment and double-support time for both feet. Contacts are
detected by thresholding each foot's total force at 5\,\% of its maximum, and
strides outside 0.5--2.5\,s are discarded.
...
Section~\ref{sec:exp}. The reported model uses the bidirectional block,
