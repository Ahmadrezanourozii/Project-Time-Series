...
rate, and the resulting multivariate series carries the stride-timing
irregularity, shortened swing phase and left--right asymmetry that characterise
parkinsonian gait \cite{hausdorff2007, frenkel2005treadmill}.

Accuracies published on the PhysioNet gait database span roughly 70--99\,\%.
That spread is much wider than the differences one would expect between the
model families involved, which suggests something other than the model is
driving it. A recording of two minutes yields more than a hundred overlapping
windows, and if those windows are divided at random then windows of one subject
end up in both the training and the test set. A classifier can then succeed by
recognising the individual instead of the disease -- a textbook case of leakage
\cite{kaufman2012leakage}, and one of the failure modes repeatedly documented in
medical machine learning \cite{varoquaux2022machine}. This report therefore asks
two questions at once: how well a state-space model classifies these signals,
and how much of any reported accuracy is an artefact of the split.
